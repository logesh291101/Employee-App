...
class EIPService {
  EIPService({http.Client? client}) : _client = client ?? http.Client();
  ...
  EIPModel _parseResponse(http.Response response) {
    final body = _decodeBody(response);
    final apiMessage = body['message'] as String?;

    if (response.statusCode == 200) {
      return EIPModel.fromJson(body);
    }
    ...
    switch (statusCode) {
      ...
      default:
        ...
        return ApiException(
          apiMessage?.isNotEmpty == true
              ? apiMessage!
              : 'Failed to fetch employee impact points. Please try again.',
        );
    }
  }
}
